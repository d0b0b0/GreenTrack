...
import { SourceDonut, WeeklyBars } from '../components/Charts'
    ...
    <>
      ...
      <div className="dash-grid">
        ...
        <div className="card kpi col-4">
          ...
          <div className="stat-value">{fmtKg(monthly)}</div>
          <div className="stat-sub">
            {activities.length ? `Ціль — ${profile.monthlyGoal} кг` : 'Почніть логувати активності'}
          </div>
        </div>
        <div className="card kpi col-4">
          <span className="kpi-ico">🌿</span>
          <div className="stat-label">Еко-рейтинг</div>
          <div className="stat-value">{activities.length ? `${rating}/100` : '—'}</div>
        ...
        <div className="card col-4">
          ...
          <div className="progress thin" style={{ marginBottom: '0.4rem' }}>
            <span style={{ width: `${lvl.pct}%` }} />
          </div>
          <div className="muted" style={{ fontSize: '0.78rem' }}>
            {lvl.next ? `Ще ${lvl.toNext} балів до «${lvl.next.title}»` : 'Максимальний рівень!'}
          </div>
          <hr style={{ border: 'none', borderTop: '1px solid var(--border)', margin: '1rem 0' }} />
          <div className="stat-label">Місячна ціль</div>
          <div className="progress" style={{ margin: '0.6rem 0 0.4rem' }}>
            <span style={{ width: `${goalPct}%`, background: goalColor }} />
          </div>
