  ...
  return (
    <div className="dash-grid stagger">
      <div className="card col-3 kpi">
        ...
        <div className="stat-value"><Counter value={round1(total)} decimals={total % 1 ? 1 : 0} /> <span style={{ fontSize: '0.9rem' }}>кг</span></div>
      </div>
      <div className="card col-3 kpi">
        <span className="kpi-ico">📅</span>
        <div className="stat-label">Цей місяць</div>
        <div className="stat-value"><Counter value={round1(monthly)} decimals={monthly % 1 ? 1 : 0} /> <span style={{ fontSize: '0.9rem' }}>кг</span></div>
        {prevMonth > 0 && (
      ...
      <div className="card col-3 kpi">
        ...
        <div className="stat-value"><Counter value={round1(avgPerDay)} decimals={1} /> <span style={{ fontSize: '0.9rem' }}>кг</span></div>
      </div>
      <div className="card col-3 kpi">
        <span className="kpi-ico">🧮</span>
          ...
          {CATEGORIES.map((c) => {
            ...
            return (
              <div className="profile-row" key={c}>
                <span className="k">{CATEGORY_META[c].icon} {c}</span>
